...
const BalanceModel = require('./models/balance.js');
        ...
        if (interaction.options.getString('currency') === 'cash') 
        {
          /*topCashBalances.forEach((balance, index) => {
            ...
            Promise.all(users.map(async (user) => {
              ...
              {
                if(interaction.user.id == balance.user_id)
                {
                ...
                return baltopEmbed.setDescription("This guild is not in the DB");
              }
            }));
          });*/
          for (let index = 0; index < topCashBalances.length; index++) {
            const balance = topCashBalances[index];
            const user = guild.members.cache.get(balance.user_id);
