...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.options.getSubcommand() === "embedcolor") {
      ...
      {
        interaction.editReply("The color introduced is not a HEX color.")
      }else{
        userRecord.embed_color = interaction.options.getString("color");
        await userRecord.save();
        interaction.editReply(`The embed color has been changed to ${interaction.options.getString('color')}`)
      ...
      if(interaction.options.getBoolean("ephemeral") == true)
      {
        ...
        await userRecord.save();
        interaction.editReply("Ephemeral messages are now enabled.")
      }else{
        userRecord.ephemeral_message = false;
        await userRecord.save();
        interaction.editReply("Ephemeral messages are now disabled.")
      }
    } 
  },
};
