  ...
  async execute(interaction) {
    ...
    try {
          ...
          for (const itemRole of matchingItemRoles) {
            ...
            if (timeElapsed < cooldown) {
              ...
              const remainingTime = moment
                ...
                .format("HH:mm:ss");
              receivedEmbed.addFields({
                name: `Item ${itemRole.item_to_recieve} not redeemed`,
                value: `You can redeem again in ${remainingTime} Hours.`,
              });
            } else {
      ...
      if (balanceIncomeRoles.length > 0 || itemIncomeRoles.length > 0) {
        ...
        receivedEmbed.setTitle("Redeemed Successfully");
        receivedEmbed.setDescription("Redeemed successfully.");
      } else {
        receivedEmbed.setColor(`${embedColors.GENERAL_COLORS.RED}`);
        receivedEmbed.setTitle("Redeem Failed");
        receivedEmbed.setDescription("You don't have any redeemable roles.");
      }
